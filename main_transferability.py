...
from keepfit.transferability.modeling.adapters import CrossFundus
...
def init_adapter(model, args):
    ...
    if args.method == "CrossFundus":         
        print("Transferability by CrossFundus Adapter...", end="\n")         
        adapter = CrossFundus(model,  args.setting["targets"], tta=args.tta, fta=args.fta, domain_knowledge=args.domain_knowledge, train=False)     
...
def process(args):
    ...
    for iFold in range(args.folds):
        ...
        # 模型  model
        print('load_model from: ', args.weights_path)
        print('checkpoint: ',args.load_weights )
        
        model = FLAIRModel(from_checkpoint=True)
        #model, preprocess = clip.load("RN50") #RN50
...
def main():
    ...
    parser.add_argument('--balance', default=True, type=lambda x: (str(x).lower() == 'true'))          # 是否平衡数据集  Balanced dataset?
    ...
    parser.add_argument('--experiment', default='13_FIVES',
                        help='02_MESSIDOR - 13_FIVES')            # 实验使用的数据集  data set used in the experiment
    parser.add_argument('--experiment_test', default='',
                        help='02_MESSIDOR',
                        type=lambda s: [item for item in s.split(',')])                                             # OOD实验
    parser.add_argument('--method', default='CrossFundus',
                        help='CrossFundus')                                                             # 模型迁移方式 ZS、FT、LP  adaptation method
    ...
    parser.add_argument('--weights_path', default='/data/yifanchang/ULD/models/MM-Retinal/KeepFIT/KeepFIT-CFP/results/pretraining/flair+MM/flair_MM.pth ',
    ...
    parser.add_argument('--knowledge_dict', default=True, type=lambda x: (str(x).lower() == 'true'))   # 是否启用知识caption  use domain knowledge caption?
