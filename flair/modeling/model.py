...
class FLAIRModel(torch.nn.Module):
    def __init__(self, vision_type='resnet_v1', bert_type='emilyalsentzer/Bio_ClinicalBERT', vision_pretrained=True,
        ...
        if from_checkpoint:
            print('from checkpoint',self.weights_path)
    ...
    def compute_text_embeddings(self, categories, domain_knowledge=True):
    ...
    def compute_text_embeddings_gl(self, categories, domain_knowledge=True):
        # Obtain text embeddings per class
        text_embeds_dict = {}
        global_text_embed_dict = {}
        '''print('categories', categories)
        print('definitions', definitions)'''
        ...
        for iKey in range(len(categories)):
            ...
            text_embeds_dict[categories[iKey]] = text_embeds.mean(0).unsqueeze(0)
            global_text_embed_dict [categories[iKey]] = text_embeds.unsqueeze(0)

        text_embeds_dict = text_embeds_dict
        text_embeds = torch.concat(list(text_embeds_dict.values()))
        local_text_embeds = list(global_text_embed_dict.values())

        return text_embeds_dict, text_embeds, local_text_embeds
...
class VisionModel(torch.nn.Module):
    def __init__(self, vision_type='resnet', pretrained=True, proj_dim=512, proj_bias=False, projection=True,
        ...
        #weights = './flair/modeling/flair_pretrained_weights/flair_resnet.pth'             
    ...
    def compute_text_attn(self, pixel_values, text_feat):
        ...
        feat_map = self.model.layer2(x)
        #print('lay 2', x.shape)  
        N,C,H,W  = feat_map.shape
        #print('N,C,H,W', N,C,H,W)
        feat_map = feat_map.reshape(feat_map.shape[0], feat_map.shape[1], feat_map.shape[2] * feat_map.shape[3]).permute(0, 2, 1)  # NCHW -> (HW)NC N(HW)C
        #print(feat_map.shape, text_feat.shape)
        attn = torch.matmul(feat_map, text_feat.permute(1, 0)) #N HW K
        attn = F.softmax(attn, dim=2)
        feat_vision = torch.matmul(attn, text_feat) #N HW C
        feat_vision = feat_vision.permute(0, 2, 1).reshape(N,C,H,W)

        x = self.model.layer3(feat_vision)         
        x = self.model.layer4(x) 
        x = self.model.avgpool(x)
        x = torch.flatten(x, 1)
        x = self.model.fc(x)  
        x = self.projection_head_vision(x)
